# soc-lab-portfolio
SOC Analyst Lab Portfolio | Hands-on cybersecurity projects focused on log analysis, SIEM, incident response, and threat detection
# 🛡️ SOC Analyst Lab Portfolio

This repository showcases my hands-on cybersecurity learning journey focused on **Security Operations Center (SOC)** and **Governance, Risk, and Compliance (GRC)** concepts.

The goal of this portfolio is to demonstrate practical skills in:
- Log analysis
- Threat detection
- Incident response
- Security monitoring
- Risk awareness and governance fundamentals

---

## 🚀 About Me

I am a CompTIA Security+ certified cybersecurity graduate with a strong interest in **SOC operations, cloud security, and identity & access management (IAM)**.

Through this repository, I am documenting my hands-on labs and translating them into real-world security scenarios.

 
## 📂 Projects
 
| # | Project | Status |
|---|---|---|
| 01 | [SOC Fundamentals](./01-soc-fundamentals/README.md) | ✅ Complete |
 
---
 
## 🤝 Connect
 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/joshiyashcyber/)
[![Medium](https://img.shields.io/badge/Medium-000000?style=flat&logo=medium&logoColor=white)](https://medium.com/p/fffc7bda12b8?postPublishedType=initial)
